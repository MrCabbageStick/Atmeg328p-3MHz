...
use arduino_hal::{Adc, I2c, Peripherals, hal::{delay::Delay, usart::Usart}, prelude::_unwrap_infallible_UnwrapInfallible, usart::Baudrate};
...
fn panic(_info: &PanicInfo) -> ! {
    ...
    let line_n = &_info.location().unwrap().line();

    ufmt::uwrite!(&mut serial, "PANIC!\r\n").unwrap_infallible();

    if let Some(location) = _info.location(){
    //     ufmt::uwrite!(&mut serial, "PANIC\r\n").unwrap_infallible();
        ufmt::uwrite!(&mut serial, "Line: {}\r\n", line_n).unwrap_infallible();
    ...
    loop {
        // dp.PORTB.portb().modify(|r, w| w.pb1().bit(r.pb1().bit() ^ true));
        led.toggle();
